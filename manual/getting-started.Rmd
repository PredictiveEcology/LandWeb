...
```bash
## get the image
docker pull achubaty/landweb-standalone:2023-03-31
...
docker run -d -it \
  ...
  --name LandWeb-standalone_2023-03-31 \
  achubaty/landweb-standalone:2023-03-31
```
...
```bash
docker stop landweb-standalone:2023-03-31
docker rm landweb-standalone:2023-03-31
```
...
Download[^1] and install `Rtools` version 4.2 *as administrator*.
During installation, be sure to check the option to add `Rtools` to your `PATH`.
...
To build software, you will need the Xcode command line tools[^5], which include various compilers and `git` version control software.
...
2. Create a GitHub (<https://github.com>) account if you don't already have one, and configure a Personal Access Token (PAT).
    ...
    a.  Create a GitHub PAT following the instructions[^GH_PAT];
    b.  Be sure to uncheck all scopes.
    c.  Copy this token and save it in a text file in your home directory called `.Renviron`.
        ...
        If you are not sure where your home directory is located, run the following in an R session: 
...
1. Download and install R version 4.2.3.
    ...
    ***Ubuntu Linux**
    1. Add the CRAN `apt` repository to get the required version of R.
    ...
    :::{.rmdcaution}
    **This is out of date, as R 4.3 is the most recent version of R.**
    ...
    ```bash
    ## install R
    sudo apt-get -y install r-base r-base-dev r-cran-littler 
    sudo apt-get build-dep -y r-cran-rjava r-cran-tkrplot
...
3. (optional) On Linux, configure `ccache` to speed up R package re-installation and updates[^6].
...
[^6]: <http://dirk.eddelbuettel.com/blog/2017/11/27/#011_faster_package_installation_one>
...
**NOTE:** the `app` and `deploy` submodule repos are *private*.
...
data.frame(
  directory = fs::dir_ls("~/GitHub/LandWeb", recurse = 0, type = c("directory", "symlink")) |>
    fs::path_file() |>
    paste0("/"),
  description = c(
    "additional R helper scripts",                             ## R/
    "web-app code (git submodule; private repo)",              ## app/
    "all per-run and per-study area cache files stored here",  ## cache/
    "web-app deployment code (git submodule; private repo)",   ## deploy/
  ...
  knitr::kable(caption = "LandWeb project directory structure",
