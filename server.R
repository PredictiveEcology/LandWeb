function(input, output, session) {
  ...
  output$seralStagePctCoverMap <- renderPlot({
    regionNum <- if (is.null(input$regionSelector)) 139 else as.numeric(input$regionSelector)
    num <- match(regionNum, availableRegions)
    toMap <- get(paste0("seralMapExampleRegion", num))
    Plot(toMap, title = FALSE, new = TRUE, speedup = 10,
         cols = rev(c("forestgreen", "olivedrab3", "goldenrod3", "grey")))
  })
  ...
  output$seralStagePctCover <- renderPlot({
    ...
    ggplot(data = seralStageData[ecoregion == regionNum, ],
      ...
      scale_fill_manual(name = "Seral Stage",
                        values = c("forestgreen", "olivedrab3", "goldenrod3", "grey"),
  ...
  output$vegTypePctCoverMap <- renderPlot({
    ...
    Plot(toMap, title = FALSE, new = TRUE, speedup = 10,
         cols = rev(c("white", "yellow3", "grey70", "white")))
  })
  ...
  output$vegTypePctCover <- renderPlot({
    ...
    ggplot(data = vegTypeData[ecoregion == regionNum,],
      ...
      scale_fill_manual(name = "Vegetation Type",
                        values = c("white", "yellow3", "grey70", "white"),
  ...
  output$patchSizesFig5 <- renderPlot({
    ...
    figure5 <- ggplot(data = figure5basedata, aes(x = x, y = y)) +
      ...
      scale_x_continuous("", limits = c(-0.35, 1.99), breaks = 0:1, labels = 0:1) +
      scale_y_continuous("", limits = c(-0.4, 4.3), breaks = 1:4, labels = 1:4) +
      theme_bw() +
  ...
  output$patchSizesFig6 <- renderPlot({
    ...
    figure6 <- figure6 + 
      annotation_custom(grob = figure6_1_1gp,
                        xmin = 0.01, xmax = 1.99,
                        ymin = 3.05, ymax = 3.99) +
  ...
  output$patchSizesFig6Raster <- renderPlot(height = 800,  {
    ...
    Plot(aspenLeading, spruceLeading, new = TRUE, legend = FALSE, title = FALSE)
