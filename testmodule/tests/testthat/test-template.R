...
#    `functionName` is a function's name in your module (e.g., `testmoduleEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/yonluo/Documents/GitHub/LandWeb/testmodule/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("testmodule")
  ...
  parameters <- list(
    ...
    testmodule = list(.saveInitialTime = NA)
  )
  ...
  if (exists("testmoduleEvent1", envir = .GlobalEnv)) {
    simOutput <- testmoduleEvent1(mySim)
  } else {
    simOutput <- mySim$testmoduleEvent1(mySim)
  }
  ...
  if (exists("testmoduleEvent2", envir = .GlobalEnv)) {
    simOutput <- testmoduleEvent2(mySim)
  } else {
    simOutput <- mySim$testmoduleEvent2(mySim)
  }
