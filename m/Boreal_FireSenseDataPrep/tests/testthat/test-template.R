...
#    `functionName` is a function's name in your module (e.g., `Boreal_FireSenseDataPrepEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/jemar270/Documents/GitHub/LandWeb/m/Boreal_FireSenseDataPrep/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("Boreal_FireSenseDataPrep")
  path <- list(modulePath = "C:/Users/jemar270/Documents/GitHub/LandWeb/m",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    Boreal_FireSenseDataPrep = list(.saveInitialTime = NA)
  )
  ...
  if (exists("Boreal_FireSenseDataPrepEvent1", envir = .GlobalEnv)) {
    simOutput <- Boreal_FireSenseDataPrepEvent1(mySim)
  } else {
  ...
  if (exists("Boreal_FireSenseDataPrepEvent2", envir = .GlobalEnv)) {
    simOutput <- Boreal_FireSenseDataPrepEvent2(mySim)
  } else {
