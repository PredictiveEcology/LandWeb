...
doEvent.Boreal_FireSenseDataPrep = function(sim, eventTime, eventType, debug = FALSE) {
  switch(
    ...
    event1 = {
      ...
      # e.g.,
      # sim <- scheduleEvent(sim, time(sim) + increment, "Boreal_FireSenseDataPrep", "templateEvent")
...
.inputObjects <- function(sim) {
  ...
  # Filenames
  nbdbPointFilename <- "NFDB_point_20171106"
  
  # Also extract
  nfdbPointAlsoExtract <- basename(paste0(tools::file_path_sans_ext(nbdbPointFilename), ".", c("dbf", "prj", "sbn", "sbx", "shx")))
  
  sim$NFDB_point <- Cache(prepInputs,
                          targetFile = nbdbPointFilename,
