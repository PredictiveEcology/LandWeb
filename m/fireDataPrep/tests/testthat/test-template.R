...
#    `functionName` is a function's name in your module (e.g., `fireDataPrepEvent1`).
# 2. Copy this file to the tests folder (i.e., `/Users/stevec/Documents/GitHub/LandWeb/fireDataPrep/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("fireDataPrep")
  ...
  parameters <- list(
    ...
    fireDataPrep = list(.saveInitialTime = NA)
  )
  ...
  if (exists("fireDataPrepEvent1", envir = .GlobalEnv)) {
    simOutput <- fireDataPrepEvent1(mySim)
  } else {
    simOutput <- mySim$fireDataPrepEvent1(mySim)
  }
  ...
  if (exists("fireDataPrepEvent2", envir = .GlobalEnv)) {
    simOutput <- fireDataPrepEvent2(mySim)
  } else {
    simOutput <- mySim$fireDataPrepEvent2(mySim)
  }
