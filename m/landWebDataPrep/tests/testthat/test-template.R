...
#    `functionName` is a function's name in your module (e.g., `landWebDataPrepEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/yonluo/Documents/GitHub/LandWeb/landWebDataPrep/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("landWebDataPrep")
  ...
  parameters <- list(
    ...
    landWebDataPrep = list(.saveInitialTime = NA)
  )
  ...
  if (exists("landWebDataPrepEvent1", envir = .GlobalEnv)) {
    simOutput <- landWebDataPrepEvent1(mySim)
  } else {
    simOutput <- mySim$landWebDataPrepEvent1(mySim)
  }
  ...
  if (exists("landWebDataPrepEvent2", envir = .GlobalEnv)) {
    simOutput <- landWebDataPrepEvent2(mySim)
  } else {
    simOutput <- mySim$landWebDataPrepEvent2(mySim)
  }
