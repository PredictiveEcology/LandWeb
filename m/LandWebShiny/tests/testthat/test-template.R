...
#    `functionName` is a function's name in your module (e.g., `LandWebShinyEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Eliot/GitHub/LandWeb/m/LandWebShiny/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("LandWebShiny")
  ...
  parameters <- list(
    ...
    LandWebShiny = list(.saveInitialTime = NA)
  )
  ...
  if (exists("LandWebShinyEvent1", envir = .GlobalEnv)) {
    simOutput <- LandWebShinyEvent1(mySim)
  } else {
  ...
  if (exists("LandWebShinyEvent2", envir = .GlobalEnv)) {
    simOutput <- LandWebShinyEvent2(mySim)
  } else {
