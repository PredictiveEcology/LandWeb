...
#    `functionName` is a function's name in your module (e.g., `initBaseMapsEvent1`).
# 2. Copy this file to the tests folder (i.e., `/Users/stevec/Documents/GitHub/LandWeb/initBaseMaps/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("initBaseMaps")
  ...
  parameters <- list(
    ...
    initBaseMaps = list(.saveInitialTime = NA)
  )
  ...
  if (exists("initBaseMapsEvent1", envir = .GlobalEnv)) {
    simOutput <- initBaseMapsEvent1(mySim)
  } else {
    simOutput <- mySim$initBaseMapsEvent1(mySim)
  }
  ...
  if (exists("initBaseMapsEvent2", envir = .GlobalEnv)) {
    simOutput <- initBaseMapsEvent2(mySim)
  } else {
    simOutput <- mySim$initBaseMapsEvent2(mySim)
  }
