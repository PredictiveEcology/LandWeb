...
test_that("test Event1 and Event2.", {
  ...
  parameters <- list(
    ...
    LandWeb_preamble = list(.saveInitialTime = NA)
  )
  ...
  if (exists("LandWeb_preambleEvent1", envir = .GlobalEnv)) {
    simOutput <- LandWeb_preambleEvent1(mySim)
  } else {
  ...
  if (exists("LandWeb_preambleEvent2", envir = .GlobalEnv)) {
    simOutput <- LandWeb_preambleEvent2(mySim)
  } else {
