loadPaulAndCASFRI <- function(paths, PaulRawFileName, PaulZipFileName, existingSpeciesLayers,
  ...
  # PaulRawFileName <- "PaulSppFilled.tif"
  # #PaulRawFileName <- basename(PaulRawFileName)
  # PaulZipFileName <- file.path(dpath, basename(PaulZipFileName))
  # 
  # browser()
  # prepInputs(targetFile = PaulRawFileName, archive = PaulZipFileName, destinationPath = dpath, 
  #            alsoExtract = "SPP_1990_100m_NAD83_LCC_BYTE_VEG_NO_TIES_FILLED_FINAL.hdr", rasterToMatch = existingSpeciesLayers,
  #            fun = "raster", pkg = "raster", overwrite = TRUE, writeCropped = TRUE, rasterDatatype = "INT2U")
  # 
  # # if (FALSE) { # this is now old
  # #   if (!file.exists(PaulRawFileName))  {
  # #     if (!file.exists(PaulZipFileName)) {
  ...
  # #       googledrive::drive_download(googledrive::as_id(file_url), path = PaulZipFileName,
  # #                                   overwrite = TRUE, verbose = TRUE)
  # #     }
  # #     unzip(PaulZipFileName, exdir = dpath, overwrite = TRUE)
  # #   }
  # # }
  # # 
