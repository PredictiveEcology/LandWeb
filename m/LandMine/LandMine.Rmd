...
fitSN <- function(sna, ros, centreCell, fireSize, 
  ...
  abs(bf$LM$perimeter.area.frac.dim - 0.007) + (sum(bf$burnedMap[])*(250^2/1e4) < 9e4) # it needs to get to above 90,000 HA for it to count
}
...
outs <- DEoptim(fitSN, lower = c(-1.75, -2, -3, -3, 3, 4), upper = c(-0.5, -0.5, -0.5, -1, 4, 5), 
        ...
        sizeCutoffs = sizeCutoffs, wholeExtentPoly = wholeExtentPoly)
...
spawnNewActive <- 10^c(-0.724547,   -1.23494,   -3.309949,   -3.403017)
sizeCutoffs <- 10^c(3.183355,    4.144677)
