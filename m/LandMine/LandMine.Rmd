...
fitSN <- function(sna, ros, centreCell, fireSizes = 10^(2:5), 
                    wholeExtentPoly, desiredPerimeterArea = 0.004) {
  ...
  res <- lapply(seq(bfs1), function(bfCount) {
    abs(log(bfs1[[bfCount]]$LM[1,"perim.area.ratio"]) -
      ...
      (sum(bfs1[[bfCount]]$burnedMap[]) < fireSizes[bfCount]) # it needs to get to above 90,000 HA for it to count
  })
  sum(unlist(res))
...
outs <- DEoptim(fitSN, lower = c(-1.75, -2, -3, -3, 3, 4), upper = c(-0.5, -0.5, -0.5, -1, 4, 5), 
        control = DEoptim.control(VTR = 0.000001, itermax = 75, cluster = cl), 
        ros = ros, centreCell = centreCell, fireSize = fireSizes, 
        wholeExtentPoly = wholeExtentPoly, desiredPerimeterArea = 0.004)
