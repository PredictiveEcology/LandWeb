...
defineModule(sim, list(
  ...
  inputObjects = bind_rows(
    expectsInput("rstStudyRegion", "RasterLayer", "A factor raster, with at least 1 column called LTHRC, representing the fire return interval in years"),
    expectsInput("rstFlammable", "RasterLayer", "A raster layer, with 0, 1 and NA, where 0 indicates areas that are flammable, 1 not flammable (e.g., lakes) and NA not applicable (e.g., masked)")
  ),
...
LandMineInit <- function(sim) {
  ...
  sim$fireReturnInterval <- Cache(writeRaster, sim$fireReturnInterval,
                                  filename = file.path(tempdir(), "fireReturnInterval.tif"),
