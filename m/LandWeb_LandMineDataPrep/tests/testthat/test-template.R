...
#    `functionName` is a function's name in your module (e.g., `LandWeb_LandMineDataPrepEvent1`).
# 2. Copy this file to the tests folder (i.e., `/home/emcintir/Documents/GitHub/LandWeb/m/LandWeb_LandMineDataPrep/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("LandWeb_LandMineDataPrep")
  path <- list(modulePath = "/home/emcintir/Documents/GitHub/LandWeb/m",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    LandWeb_LandMineDataPrep = list(.saveInitialTime = NA)
  )
  ...
  if (exists("LandWeb_LandMineDataPrepEvent1", envir = .GlobalEnv)) {
    simOutput <- LandWeb_LandMineDataPrepEvent1(mySim)
  } else {
  ...
  if (exists("LandWeb_LandMineDataPrepEvent2", envir = .GlobalEnv)) {
    simOutput <- LandWeb_LandMineDataPrepEvent2(mySim)
  } else {
