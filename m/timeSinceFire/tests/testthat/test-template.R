...
#    `functionName` is a function's name in your module (e.g., `timeSinceFireEvent1`).
# 2. Copy this file to the tests folder (i.e., `/Users/stevec/Documents/GitHub/LandWeb/timeSinceFire/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("timeSinceFire")
  path <- list(modulePath = "/Users/stevec/Documents/GitHub/LandWeb",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    timeSinceFire = list(.saveInitialTime = NA)
  )
  ...
  if (exists("timeSinceFireEvent1", envir = .GlobalEnv)) {
    simOutput <- timeSinceFireEvent1(mySim)
  } else {
    simOutput <- mySim$timeSinceFireEvent1(mySim)
  }
  ...
  if (exists("timeSinceFireEvent2", envir = .GlobalEnv)) {
    simOutput <- timeSinceFireEvent2(mySim)
  } else {
    simOutput <- mySim$timeSinceFireEvent2(mySim)
  }
