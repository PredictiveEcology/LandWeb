...
#    `functionName` is a function's name in your module (e.g., `makeLeafletTilesEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Eliot/GitHub/LandWeb/m/makeLeafletTiles/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("makeLeafletTiles")
  ...
  parameters <- list(
    ...
    makeLeafletTiles = list(.saveInitialTime = NA)
  )
  ...
  if (exists("makeLeafletTilesEvent1", envir = .GlobalEnv)) {
    simOutput <- makeLeafletTilesEvent1(mySim)
  } else {
  ...
  if (exists("makeLeafletTilesEvent2", envir = .GlobalEnv)) {
    simOutput <- makeLeafletTilesEvent2(mySim)
  } else {
