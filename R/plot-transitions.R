...
vegTransitions <- function(vtm, ecoregion, field, studyArea, times, na.rm = FALSE) {
  ...
  transitions_df <- lapply(seq_along(times), function(yr) {
    ...
    names(lvls_vt) <- tolower(names(lvls_vt))
    if (tools::file_ext(vtm[yr]) == "grd") {
      idcol_vt <- grep("^(id)$", names(lvls_vt), ignore.case = TRUE, value = TRUE)
      valcol_vt <- grep("^(id)$", names(lvls_vt), ignore.case = TRUE, invert = TRUE, value = TRUE)
    } else {
      idcol_vt <- grep("^(id|value)$", names(lvls_vt), ignore.case = TRUE, value = TRUE)
      valcol_vt <- grep("^(id|value)$", names(lvls_vt), ignore.case = TRUE, invert = TRUE, value = TRUE)
    }
    ...
    tdf <- data.table(
      ...
      ecoregion = lvls_er[[field]][match(terra::values(ecoregion, mat = FALSE), lvls_er[[idcol_er]])],
      vegType = lvls_vt[[valcol_vt]][match(terra::values(r, mat = FALSE), lvls_vt[[idcol_vt]])],
      time = times[yr]
    ) |>
      na.omit("ecoregion")
