checkoutVersion <- function(gitHash, localRepoPath = ".", githubPATname = "GITHUB_PAT",
  ...
  return(list(repo = repo, hasUncommittedFiles = hasUncommittedFiles,
              lastCommit = lastCommit, remoteWasHTTPS = remoteWasHTTPS, sshURL = sshURL))
}
...
checkoutDev <- function(checkoutCondition) {
  ...
  if (checkoutCondition$hasUncommittedFiles) {
    git2r::reset(checkoutCondition$lastCommit, reset_type = "soft")
  }
