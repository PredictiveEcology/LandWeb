...
west <- can_provs[can_provs$NAME_1 %in% c("Alberta", "British Columbia", "Saskatchewan"), ]
```
...
plot(dmi.ansr)
shapefile(dmi.ansr, filename = file.path(dataDirDMI, "DMI_ANSR.shp"), overwrite = TRUE)
```
...
## add replorting polygons -- why do they need to rewrite shapefiles??
ml.dmi <- mapAdd(dmi.full, ml.dmi, layerName = "DMI Full", poly = TRUE,
                 filename2 = file.path(inputsDir, "DMI", "tmp_full.shp"), overwrite = TRUE)

ml.dmi <- mapAdd(dmi.e, ml.dmi, layerName = "DMI East", poly = TRUE,
                 filename2 = file.path(inputsDir, "DMI", "tmp_east.shp"), overwrite = TRUE)

ml.dmi <- mapAdd(dmi.nw, ml.dmi, layerName = "DMI West North", poly = TRUE,
                 filename2 = file.path(inputsDir, "DMI", "tmp_west_north.shp"), overwrite = TRUE)

ml.dmi <- mapAdd(dmi.sw, ml.dmi, layerName = "DMI West South", poly = TRUE,
                 filename2 = file.path(inputsDir, "DMI", "tmp_west_south.shp"), overwrite = TRUE)

ml.dmi <- mapAdd(dmi.caribou, ml.dmi, layerName = "DMI Caribou", poly = TRUE,
                 filename2 = file.path(inputsDir, "DMI", "tmp_caribou.shp"), overwrite = TRUE)

ml.dmi <- mapAdd(dmi.ansr, ml.dmi, layerName = "DMI ANSR", poly = TRUE,
                 filename2 = file.path(inputsDir, "DMI", "tmp_ansr.shp"), overwrite = TRUE)
```
...
plot(tolko_ab_s, add = TRUE, col = colours[c(2, 3, 5)])
...
```{r tolko_sk}
tolko_sk <- tolko[1, ]
plot(tolko_sk, add = TRUE, col = colours[1])
