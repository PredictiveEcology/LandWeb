...
west <- can_provs[can_provs$NAME_1 %in% c("British Columbia", "Alberta", "Saskatchewan"), ]
manitoba <- can_provs[can_provs$NAME_1 %in% c("Manitoba"), ]
```

```{r fire-return-intervals}
## LandWeb study area polygons need LTHFC data
firemap <- shapefile(file.path(inputsDir, "landweb_ltfc_v6.shp"))
...
plot(spTransform(west, crs(lp)), add = TRUE)
...
lp_mb.caribou <- postProcess(caribou, studyArea = lp_mb, useSAcrs = TRUE,
