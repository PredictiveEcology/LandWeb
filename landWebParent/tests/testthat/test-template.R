...
#    `functionName` is a function's name in your module (e.g., `landWebParentEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/yonluo/Documents/GitHub/landWebParent/landWebParent/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("landWebParent")
  path <- list(modulePath = "C:/Users/yonluo/Documents/GitHub/landWebParent",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    landWebParent = list(.saveInitialTime = NA)
  )
  ...
  if (exists("landWebParentEvent1", envir = .GlobalEnv)) {
    simOutput <- landWebParentEvent1(mySim)
  } else {
    simOutput <- mySim$landWebParentEvent1(mySim)
  }
  ...
  if (exists("landWebParentEvent2", envir = .GlobalEnv)) {
    simOutput <- landWebParentEvent2(mySim)
  } else {
    simOutput <- mySim$landWebParentEvent2(mySim)
  }
