...
#    `functionName` is a function's name in your module (e.g., `LandMineEvent1`).
# 2. Copy this file to the tests folder (i.e., `c:/Eliot/GitHub/LandWeb/LandMine/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("LandMine")
  path <- list(modulePath = "c:/Eliot/GitHub/LandWeb",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    LandMine = list(.saveInitialTime = NA)
  )
  ...
  if (exists("LandMineEvent1", envir = .GlobalEnv)) {
    simOutput <- LandMineEvent1(mySim)
  } else {
    simOutput <- mySim$LandMineEvent1(mySim)
  }
  ...
  if (exists("LandMineEvent2", envir = .GlobalEnv)) {
    simOutput <- LandMineEvent2(mySim)
  } else {
    simOutput <- mySim$LandMineEvent2(mySim)
  }
