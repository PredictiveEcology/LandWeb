...
# the cohort data is set to not allow on site regeneration  
  ...
  if(exists("biomassSuccessionLANDISWardDispersalSeeding")){
    output <- biomassSuccessionLANDISWardDispersalSeeding(mySim)
  } else {
    output <- mySim$biomassSuccessionLANDISWardDispersalSeeding(mySim) 
  }
