test_that("test growth and mortality at main simulation stage",{
  ...
  for(i in 2:493){
    if(exists("biomassSuccessionLANDISMortalityAndGrowth")){
      simOutput <- biomassSuccessionLANDISMortalityAndGrowth(mySim)
    } else {
      simOutput <- mySim$biomassSuccessionLANDISMortalityAndGrowth(mySim)
    }
