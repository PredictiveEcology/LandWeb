test_that("test biomassSuccessionLANDISInit",{
  ...
  mySim <- simInit(times=list(start=0, end=2),
                   ...
                   objects=objects,
                   paths=path)
  if(exists("biomassSuccessionLANDISInit")){
    simOutput <- biomassSuccessionLANDISInit(mySim)
  } else {
    simOutput <- mySim$biomassSuccessionLANDISInit(mySim)
  }
