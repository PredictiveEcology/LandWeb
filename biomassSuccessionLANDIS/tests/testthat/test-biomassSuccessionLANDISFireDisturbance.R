test_that("test regeneration after fire. ",{
  ...
  mySim <- simInit(times=list(start=0, end=2),
                   ...
                   paths=path)
  set.seed(1234)
  if(exists("biomassSuccessionLANDISFireDisturbance")){
    simOutput <- biomassSuccessionLANDISFireDisturbance(mySim)
  } else {
    simOutput <- mySim$biomassSuccessionLANDISFireDisturbance(mySim)
  }
