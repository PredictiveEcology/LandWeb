test_that("test universal dispersal seeding algorithm",{
  ...
  mySim <- simInit(times=list(start=0, end=2),
                   ...
                   paths=path)
  set.seed(2)
  if(exists("biomassSuccessionLANDISUniversalDispersalSeeding")){
    simOutput <- biomassSuccessionLANDISUniversalDispersalSeeding(mySim)
  } else {
    simOutput <- mySim$biomassSuccessionLANDISUniversalDispersalSeeding(mySim)
  }
