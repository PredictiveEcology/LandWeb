test_that("test no dispersal seeding algorithm",{
  ...
  mySim <- simInit(times=list(start=0, end=2),
                   ...
                   paths=path)
  set.seed(1)
  if(exists("biomassSuccessionLANDISNoDispersalSeeding")){
    output <- biomassSuccessionLANDISNoDispersalSeeding(mySim)
  } else {
    output <- mySim$biomassSuccessionLANDISNoDispersalSeeding(mySim) 
  }
