...
#    `functionName` is a function's name in your module (e.g., `fireNullEvent1`).
# 2. Copy this file to the tests folder (i.e., `/Users/stevec/Documents/GitHub/LandWeb/fireNull/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("fireNull")
  ...
  parameters <- list(
    ...
    fireNull = list(.saveInitialTime = NA)
  )
  ...
  if (exists("fireNullEvent1", envir = .GlobalEnv)) {
    simOutput <- fireNullEvent1(mySim)
  } else {
    simOutput <- mySim$fireNullEvent1(mySim)
  }
  ...
  if (exists("fireNullEvent2", envir = .GlobalEnv)) {
    simOutput <- fireNullEvent2(mySim)
  } else {
    simOutput <- mySim$fireNullEvent2(mySim)
  }
